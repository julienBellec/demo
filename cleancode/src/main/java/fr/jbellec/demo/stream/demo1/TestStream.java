...
public class TestStream {
    ...
    public static final String[] TYPE_PERSONNE_ARRAY = new String[] { "Android",
            "Hybrid", "Higher" };

    static {
        listP = Arrays.asList(new Personne(1.80, 70, "A", "Nicolas", Couleur.BLEU),
    ...
    private static void testStreamMapNotEmpty() {
    	...
    	Map<String, List<Personne>> notEmtpy = mapP.entrySet().stream()
    ...
    private static void testStreamMapToString() {
    	String test =  mapP.entrySet().stream()
    			.limit(1)
    			.map(entry -> "" +  entry.getValue().stream().map(Personne::getPrenom).collect(Collectors.joining(", ")))
    ...
    private static void testStreamReduce() {
        ...
        Double sum = sp.filter(x -> x.getPoids() > 50).map(x -> x.getPoids()).reduce(0.0d,
                (x, y) -> x + y);
    ...
    private static void testStreamReduceIfPresent() {
        ...
        Optional<Double> sum = sp.filter(x -> x.getPoids() > 250).map(x -> x.getPoids())
                .reduce((x, y) -> x + y);
    ...
    private static void testStreamCollect() {
        ...
        List<Double> ld = sp.filter(x -> x.getPoids() > 50).map(x -> x.getPoids())
